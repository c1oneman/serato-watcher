...
public class SessionBytesPublisherServiceTest {
	private EventHub hub;
	
	private TestLastMsgReceivedService testAdatService = new TestLastMsgReceivedService();
	...
	public void setup() {
		...
		// converts session files into bytes:
		SessionDirectoryFilesToBytesService sessionToBytesService = new SessionDirectoryFilesToBytesService(hub);
		// class under test - converts bytes into adat chunks
		SessionBytesPublisherService service = new SessionBytesPublisherService(hub);
	}

	@SuppressWarnings("unchecked")
	@Test public void single_track_session() {
		hub.register(testAdatService);
		
		File session = new File(TEST_SESSION_FILES_LOC + "1_song.session");
		hub.add(new SessionFileCreatedMessage(session));
		
		List<Adat> adats = getExpectedAdatResult();
	...
	@SuppressWarnings("unchecked")
	@Test
	public void windows_Session_2_Tracks() {
	...
	private List<Adat> getExpectedAdatResult() {
		return testAdatService.allMsgsRecvd
				.stream()
				.filter(AdatConsumedMessage.class::isInstance).map(AdatConsumedMessage.class::cast)
				.map(msg -> msg.adat)
				.collect(toList());
	}
